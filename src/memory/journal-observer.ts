...
async function spawnObserver(prompt: string): Promise<string> {
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on('error', (err) => {
      ...
      rejectPromise(
        new Error(`journal observer spawn failed: ${err.message}`),
      )
    })
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        ...
        return rejectPromise(
          new Error(
            `journal observer exit ${code}: ${stderr.slice(0, 300)}`,
          ),
        )
      }
      try {
        const parsed = JSON.parse(stdout) as ObserverClaudeOutput
        if (
          parsed.is_error ||
          ...
          logFail(
            `bad output: ${parsed.result ?? stderr.slice(0, 200)}`,
          )
          return rejectPromise(
            new Error(
              `journal observer bad output: ${
                parsed.result ?? stderr.slice(0, 200)
              }`,
            ),
          )
        }
