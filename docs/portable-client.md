...
The agreed hosted architecture places HeyAmigo Brain inside HeyAmigo Cloud, with private goals, memory, feedback and learning per Amigo. The future Cloud Client executes authorized tasks and returns observations and results. Its browser, platform credentials, channel sessions and local execution state remain in this private volume; Brain's authoritative agenda remains in Cloud when the Client is replaced. Hosted mode must route incoming messages to Brain and prevent the standalone bot's reply/schedule paths from duplicating Brain actions. Neither Brain nor this connection is implemented by the portable package.
