...
The bot and Chrome lifecycles are separate. `heyamigo restart` restarts the Node supervisor only; `heyamigo chrome restart` recovers the single VNC browser plus its Xvfb, x11vnc, and noVNC support stack through the packaged setup launcher. It does not install missing system packages. noVNC normally binds to 6090 directly; when nginx already owns that frontend port, the launcher detects it and restores websockify on backend port 6080 instead. Every automatic path derives the same profile at `~/.config/google-chrome-novnc`; there is no configurable alternate profile. Chrome management checks the real Linux process executable plus its CDP port/profile arguments and refuses to act when an unknown browser owns the endpoint.
