#!/usr/bin/env node
// One owner-provisioned Amigo per private volume. This is not Cloud authentication.
import { spawn } from 'node:child_process'
import { mkdir, readFile, writeFile, readdir, lstat, realpath, copyFile, constants } from 'node:fs/promises'
...
export async function openState(env, initialize = false) {
  ...
  if (
    ...
    throw new Error('This volume belongs to a different Amigo or uses an unsupported state version.')
  ...
  if (initialize) {
    const config = JSON.parse(await readFile(join(packageRoot, 'config/config.example.json'), 'utf8'))
    ...
    const access = JSON.parse(await readFile(join(packageRoot, 'config/access.example.json'), 'utf8'))
    access.users = {}
    access.groups = []
...
async function closeBrowser(browser) {
  try {
    ...
    if (endpoint.protocol !== 'ws:' || endpoint.hostname !== '127.0.0.1' || endpoint.port !== '9222')
...
async function main() {
  ...
  if (!['init', 'check', 'start', '_run'].includes(command))
    throw new Error(
      'Usage: portable-client.mjs init|check|start. Configure AMIGO_STATE_DIR, AMIGO_WORKSPACE_ID, AMIGO_AGENT_ID.',
    )
  if (command === 'init') {
    const state = await openState(process.env, true)
    console.log(JSON.stringify({ status: 'initialized', ...state.identity, connections: 'disabled' }))
    return
  }
  ...
  if (command === 'check') {
    console.log(
      JSON.stringify({
        ...
        cloudControl: 'not_connected',
      }),
    )
    return
  }
  if (process.platform !== 'linux') throw new Error('Run the portable client in its Linux container.')
  ...
  await supervise(children, 30000, process.env.AMIGO_BROWSER_ENABLED === '1' ? children[0] : undefined)
}
